...
jdbcTemplate.query("select * from schedules where user_name = ? order by updated_at desc", scheduleRowMapper(), userName);
```
#### 2. 상황에 따라 매개변수를 다양하게 받아 객체를 생성하고 싶었다.
그 때 그 때 다른 매개변수를 받아 객체를 생성하고 싶은데 생성자를 여러개 만들 수 있다는 사실을 잊고 오류를 맞이하였다.  
...
#### 3. 칼럼이 될 변수 `contents`가 데이터베이스로 매핑할 때 `content`로 인식이 되는 오류가 발생하였다.
